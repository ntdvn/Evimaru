...
class ManagerCoSoDaoTaoClassInformation extends Component {
    constructor(props){
        super(props);
        var thisLopHoc = getIdFromArray(this.props.match.params.classId, this.props.khoaHoc.lopHoc)
        ...
        this.state = {
            ...
            tinhTrang: thisLopHoc.tinhTrang,
            checkPostLopHoc: [],
    ...
    handlerPostLopHoc = id =>  {
        let { tenLop, soQuyetDinh, description, ngayBatDau, ngayKetThuc, buoiHocArr, giangVienArr, gioiHanHocVien, tinhTrang } = this.state;
        ...
        if(tinhTrang==""){
            if(!arrayResult.includes(errorArr[5])){
                arrayResult=[...arrayResult, errorArr[5]];
                checkError=true;
            }
        }
    ...
    componentWillReceiveProps(nextProps) {
        const { khoaHoc, match } = nextProps;
        var thisLopHoc = getIdFromArray(match.params.classId, khoaHoc.lopHoc);
        this.setState({
            tenLop: thisLopHoc.tenLop,
            soQuyetDinh: thisLopHoc.soQuyetDinh,
            description: thisLopHoc.description,
    ...
    render() {
        ...
        var listCauHoi, listBuoiHoc, listGiangVien, lopHoc, dataGiangViens, table, data, tinhTrangArray, optionTinhTrang;
        var thisLopHoc;
        // console.log(match);
        if(!JSONisEmpty(khoaHoc)){
            if(khoaHoc.hasOwnProperty('lopHoc')){
                ...
                tinhTrangArray = ["Chưa mở đăng ký", "Đang mở đăng ký", "Đã đóng đăng ký", "Đang học", "Đã kết thúc"]
                optionTinhTrang = tinhTrangArray.map((tinhTrang,i) => {
                    return (
                        <option value={tinhTrang} key={i}>{tinhTrang}</option>
                    )
                })
                ...
                return (
                    <div>
                        ...
                        <div className="row mb-3">
                            ...
                            <div className="container-fluid what-you-learn-container mb-3">
                                <h3>Tình trạng: {thisLopHoc.tinhTrang}</h3>
                                <div className="row">
                                    <div className="col-md-12">
                                        <select
                                            id="tinhTrang"
                                            ...
                                            required
                                        >
                                            <option disabled value="">Chọn nhóm khóa học</option>
                                            {optionTinhTrang}
                                        </select>
                                    </div>
                                </div>
                        ...
                        { errorPanel }
                        <Button color="primary" onClick={this.handlerPostLopHoc.bind(this, thisLopHoc._id)}>Sửa thông tin lớp</Button>
                        {errors.message && (
                            <div className="alert alert-danger" role="alert">
                                {errors.message}
                            </div>
...
export default withRouter(ManagerCoSoDaoTaoClassInformation);
